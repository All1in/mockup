...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (unsigned.ok) {
      // Провал, а не попередження. Публічний бакет робить усю перевірку прав
      // у /files декоративною: документи компаній читає будь-хто, хто знає
      // URL. Скрипт, який повідомляє про це й виходить з кодом 0, автоматика
      // прочитає як успіх — а запускати його буде саме вона.
      throw new Error(
        `об'єкт доступний БЕЗ підпису (${unsigned.status}). Бакет публічний — зроби його приватним, ` +
          'інакше документи компаній читає будь-хто.',
      );
    }
